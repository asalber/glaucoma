...
lda.glaucoma(input) %>% kable() %>% kable_styling(bootstrap_options = "hover", full_width = F)
```

### Regresión logística
```{r logit-glaucoma-all-layers-all-cells}
logit.glaucoma(input) %>% kable() %>% kable_styling(bootstrap_options = "hover", full_width = F)
```
...
lda.stages(input) %>% kable() %>% kable_styling(bootstrap_options = "hover", full_width = F)
```
...
input %>% ggplot(aes(x = GCL.Celda4.4, col= Estadio)) + geom_density()
```
