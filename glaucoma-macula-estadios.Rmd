...
prepare.input.glaucoma <- function(df, layers){
  input <- df %>%
    # Filter layer
...
prepare.input.stages <- function(df, layers){
  input <- df %>%
    # Filter glaucoma
    filter(Glaucoma == "Y") %>%
    # Filter layer
...
input <- prepare.input.glaucoma(df, layers = "INL")
model.json <- net.arch.glaucoma(ncol(input)-1)
summary(model_from_json(model.json))
```
...
input <- prepare.input.stages(df, layers = "INL")
model.json <- net.arch.stages(ncol(input)-1)
summary(model_from_json(model.json))
```
...
input <- prepare.input.glaucoma(df, layers = c("RNFL", "GCL", "INL"))
model.json <- net.arch.glaucoma(ncol(input)-1)
summary(model_from_json(model.json))
```
...
input <- prepare.input.stages(df, layers = c("RNFL", "GCL", "INL"))
model.json <- net.arch.stages(ncol(input)-1)
summary(model_from_json(model.json))
```
