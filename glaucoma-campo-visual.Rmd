...
data <- read_csv('data/campo-visual.csv', col_types = cols(Estadio = "f"))
...
data.glaucoma <- data %>% filter(Estadio != 'Sano')
```

```{r data-filtering, results='hide'}
data <- data %>% # Eliminar los casos con errores mayores del 16%
  filter(Error <= 16)
```
...
data.glaucoma[, "Estadio.CV"] <- ordered(clusters$cluster)
...
k <- kendall(data.glaucoma[, c("Estadio", "Estadio.CV")], correct = T)
k
```
